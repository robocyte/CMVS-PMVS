...
public:
    ...
    void grabTex(const int index, const int level, const Vec4f& coord, const Vec4f& pxaxis, const Vec4f& pyaxis, const Vec4f& pzaxis,
...
{
    for (int index = 0; index < m_num; ++index) if (getMask(coord, index, level) == 0) return 0;

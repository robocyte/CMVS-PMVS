...
void Soption::init(const std::string prefix, const std::string option)
{
    ...
    while (1)
    {
        ...
        else if (name == "timages")
        {
            ...
            if (m_tflag == -1)
            {
                ...
                for (int i = firstimage; i < lastimage; ++i) m_timages.push_back(i);
            } else if (0 < m_tflag)
            {
            ...
            {
                std::cerr << "tflag is not valid: " << m_tflag << std::endl;   exit (1);
            }
        } else if (name == "oimages")
        {
            ...
            if (m_oflag == -1)
            {
                int firstimage, lastimage;
                ifstr >> firstimage >> lastimage;
                for (int i = firstimage; i < lastimage; ++i) m_oimages.push_back(i);
            } else if (0 <= m_oflag)
            {
            ...
            {
                std::cerr << "oflag is not valid: " << m_oflag << std::endl;   exit (1);
            }
    ...
    {
        std::cerr << "m_tflag and m_oflag not specified: " << m_tflag << ' ' << m_oflag << std::endl;
        exit (1);
    }
    ...
    if (m_useBound) initBindexes(sbimages);

    std::cerr << "--------------------------------------------------" << std::endl
              << "--- Summary of specified options ---" << std::endl;
    std::cerr << "# of timages: " << (int)m_timages.size();
    if (m_tflag == -1)      std::cerr << " (range specification)" << std::endl;
    else                    std::cerr << " (enumeration)" << std::endl;
    std::cerr << "# of oimages: " << (int)m_oimages.size();
    if (m_oflag == -1)      std::cerr << " (range specification)" << std::endl;
    else if (0 <= m_oflag)  std::cerr << " (enumeration)" << std::endl;
    else if (m_oflag == -2) std::cerr << " (vis.dat is used)" << std::endl;
    else if (m_oflag == -3) std::cerr << " (not used)" << std::endl;
    ...
    std::cerr << "level: " << m_level << "  csize: " << m_csize << std::endl
              ...
              << "useVisData: " << m_useVisData << "  sequence: " << m_sequence << std::endl;
    std::cerr << "--------------------------------------------------" << std::endl;
}
...
void Soption::initOimages(void)
{
    ...
    {
        int index0;
        auto ite0 = m_dict.find(c);

        if (ite0 == m_dict.end()) index0 = -1;
        else                      index0 = ite0->second;
        ...
        for (int i = 0; i < itmp; ++i)
        {
            int itmp2;
            ifstr >> itmp2;
            if (index0 != -1 && m_dict.find(itmp2) == m_dict.end()) m_oimages.push_back(itmp2);
        }
    }
    ifstr.close();
...
void Soption::initVisdata2(void)
{
    ...
    for (int i = 0; i < (int)images.size(); ++i) dict2[images[i]] = i;

    std::ifstream ifstr;
    ifstr.open(svisdata.c_str());
    if (!ifstr.is_open())
    {
        std::cerr << "No vis.dat although specified to initVisdata2: " << std::endl << svisdata << std::endl;
        exit (1);
    }
    ...
    for (int c = 0; c < num2; ++c)
    {
        ...
        for (int i = 0; i < itmp; ++i)
        {
            ...
            if (ite1 == dict2.end()) index1 = -1;
            else                     index1 = ite1->second;

            if (index0 != -1 && index1 != -1) m_visdata2[index0].push_back(index1);
        }
    }
    ...
    for (int y = 0; y < num; ++y)
    {
        ...
        for (int x = 0; x < (int)m_visdata2[y].size(); ++x) m_visdata[y][m_visdata2[y][x]] = 1;
    }
...
void Soption::initBindexes(const std::string sbimages)
{
    ...
    for (int i = 0; i < itmp; ++i)
    {
        ...
        if (m_dict.find(itmp0) != m_dict.end()) m_bindexes.push_back(m_dict[itmp0]);
    }
    ifstr.close();
}

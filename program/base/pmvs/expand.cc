...
Cexpand::Cexpand(CfindMatch& findMatch)
    : m_fm(findMatch)
{
}
...
void Cexpand::run()
{
    ...
    const int trial = std::accumulate(m_ecounts.begin(),  m_ecounts.end(),  0);
...
void Cexpand::findEmptyBlocks(const Ppatch& ppatch, std::vector<std::vector<Vec4f>>& canCoords)
{
    ...
    m_fm.m_pos.findNeighbors(patch, neighbors, 1, 4.0f);
...
float Cexpand::computeRadius(const Patch::Cpatch& patch)
{
    ...
    {
        std::cerr << "units size less than minnum: " << (int)units.size() << ' ' << minnum << std::endl;
        std::cout << (int)patch.m_images.size() << std::endl;
        std::exit (1);
    }
#endif

    std::nth_element(vftmp.begin(), vftmp.begin() + minnum - 1, vftmp.end());
    // Threshold is the second smallest value with some margin ??? critical
...
int Cexpand::checkCounts(Patch::Cpatch& patch)
{
    ...
    auto begin2 = patch.m_grids.begin();
    ...
    if (m_fm.m_depth <= 1)
    {
        if (empty < m_fm.m_minImageNumThreshold && full != 0)   return 1;
        else                                                    return 0;
    } else
    {
        if (empty < m_fm.m_minImageNumThreshold - 1 && full != 0)   return 1;
        else                                                        return 0;
    }
}
...
int Cexpand::updateCounts(const Cpatch& patch)
{
    ...
    {
        auto begin  = patch.m_vimages.begin();
        auto end    = patch.m_vimages.end();
        auto begin2 = patch.m_vgrids.begin();
